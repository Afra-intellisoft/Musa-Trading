...
class PaymentReplacementWizard(models.TransientModel):
    ...
    bank_id = fields.Many2one('res.bank', string='Bank', related='original_check.payment_id.Bank_id', readonly=True)
    ...
    def confirm(self):
        check = self.env['check_followups.check_followups'].browse(self._context['active_id'])
        if check.state not in ['under_collection', 'rdc','approve_manger']:
    ...
    def _validate_number(self):
        number = sum(self.check_replacement_ids.mapped('number'))
        if number != self.original_check.check_no:
            raise ValidationError('Check No Not equal check no of replacement payments')
